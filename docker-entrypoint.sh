...
if [ "$1" = 'start-soapui' ]; then
    ...
    if [ -z "$MOCK_SERVICE_PATH" ]; then
        ...
        su-exec "${SOAPUI_USER}" mockservicerunner.sh -Djava.awt.headless=true -p 8080 -m $MOCK_SERVICE_NAME $PROJECT <&3 &
    else
        echo "Starting Mock-service=$MOCK_SERVICE_NAME using url-path=$MOCK_SERVICE_PATH from SoapUI-project=$PROJECT"
        su-exec "${SOAPUI_USER}" mockservicerunner.sh -Djava.awt.headless=true -p 8080 -m $MOCK_SERVICE_NAME -a $MOCK_SERVICE_PATH $PROJECT <&3 &
    fi
